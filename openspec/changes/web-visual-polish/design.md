...
### D8 · Portada pública en `/`
`AuthGate` muestra `features/landing/LandingPage` cuando no hay sesión y la ruta es `/` (las demás rutas privadas siguen redirigiendo al login). La portada tiene cabecera propia (logo, anclas a Servicios, Cómo funciona y Planes, botones Ingresar y Crear cuenta), hero con la frase de la marca y una tarjeta ilustrativa (KPI de ejemplo, gráfica `ui/GraficaBarras` compartida con el login, dos avisos), seis tarjetas de servicios (inventario y movimientos, rentabilidad, alertas, órdenes copiloto, proveedores y listas, app Android), tres pasos, tres planes armados con `LIMITES_PLAN` de shared y un pie con la referencia al trabajo final. Con sesión, `/` sigue siendo el panel. Alternativa descartada: una ruta `/bienvenida` aparte; Franco pidió la portada "al principio" y así el enlace público es la raíz.

### D6 · Tests y verificación
`apps/web` suma `vitest` y `jsdom` (`test: vitest run`, `environment: jsdom`): `lib/tema.test.ts` (lectura con y sin `localStorage`, aplicación del atributo, preferencia del sistema) y `ui/useContador.test.ts` (llega al valor final; con `reduced-motion` devuelve el final de inmediato). `pnpm test` en la raíz ya es recursivo, así que CI los corre sin cambios en el workflow. Verificación visual: en el navegador integrado con la sesión de Franco, para 360, 768, 1024 y 1280 px y para Portada, Login, Inicio, Inventario, Alertas, Órdenes y Nueva orden, ejecutar `document.documentElement.scrollWidth === document.documentElement.clientWidth` y registrar el resultado por pantalla y ancho en `openspec/changes/web-visual-polish/verificacion.md` (el navegador integrado no guarda capturas como archivos; el registro se archiva con el change). Tema claro: misma comprobación para Portada, Inicio y Alertas.
